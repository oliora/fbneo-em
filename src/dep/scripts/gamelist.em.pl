...
foreach my $filename ( @Filelist ) {
	...
	while ( my $line = <INFILE> ) {
		...
		if ( $line =~ /struct\s+BurnDriver([D|X]?)\s+(\w+)(.*)/ ) {
			...
			$struct =~ /\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0)\s*,\s*(".*"|null|0),\s*(L".*"|null|0)\s*,\s*(L".*"|null|0)\s*,\s*(L".*"|null|0)\s*,\s*(L".*"|null)\s*,\s*(\d|[^,]*),\s*(\d|[^,]*),\s*(\d|[^,]*),\s*(\d|[^,]*),\s*(\d|[^,]*),\s*(\d|[^,]*),\s*(\d|[^,]*)/si;
			...
			while ( my $line = <INFILE2> ) {		
				...
				if ( $line =~ /struct\s+BurnRomInfo([D|X]?)\s+(\w+)(.*)/ ) {								
					# The description for the current rom		
					my $romdesc = $2;
					my $struct = $3;
					$rominfo =~ s/RomInfo/RomDesc/ig;			
					if ($rominfo eq $romdesc) {										
...
if ( $Listfile ) {
	...
	foreach my $name ( @Driverlist ) {
		...
		$Text = substr( $Drivers{$name}[2], 0, 999);
		print OUTFILE "| $Text";
		...
		$Text = substr( $Drivers{$name}[11], 0, 999);
		print OUTFILE "| $Text";
		$column += int( ( 2 + length( $Text )) / 8 );
		while ( $column < 22 ) {
			print OUTFILE "\t";
		...
		$Text = substr( $Drivers{$name}[12], 0, 999);
		print OUTFILE "| $Text";
		$column += int( ( 2 + length( $Text )) / 8 );
		while ( $column < 22 ) {
			print OUTFILE "\t";
